...
#define FINAL_PART2_H_

namespace final_part2 {
...
class SyntheticClean : public CleanMold {
public: ~SyntheticClean() { DTORF("~SyntheticClean "); }
	void clean(const string& metal) {
		cout << "  Clean mold of Synthetic: ozone wash.\n";
	}
};
...
CleanMold* CleanMold::getCleaning(map<string,string>& order) {	// FM.
	...
	if(order["plastic"] == "Styrene")			return new SyntheticClean;
	if(order["plastic"] == "Nylon66")			return new SyntheticClean;
...
#define ProcessInherit ProcessOrderP1 // Pedagogy: successively replace with 0,1,2,3...
...
protected: // Template Method methods.
	...
	virtual void loadAdditives(map<string,string>& order) {
		using namespace decorator;
...
class SyntheticOrder : public ProcessInherit {
