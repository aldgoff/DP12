...
bool isColor(string name) { return (name == "black" || name == "brown" || name == "red" || name == "orange" || name == "yellow" || name == "green" || name == "blue" || name == "violet" || name == "gray" || name == "white"); }
bool isAddictive(string name) { return (name == "AntiBacterial" || name == "Hydrophilic" || name == "UVInhibiter" || name == "MicroFibers"); }
...
namespace adapter {			// DP 2.
	...
	public:
		...
		static CAdapterClean* makeObject(string type, string metaltype = "");
	};
	...
	class CCleanSynthetics : public CAdapterClean	{
	public:
		~CCleanSynthetics() { DTORF("~CCleanSynthetics "); }
		void clean() { cout << "    Clean synthetic mold: ozone wash.\n"; }
	};

	CAdapterClean* CAdapterClean::makeObject(string type, string metaltype) {
		if (type == "ABS")           return new CCleanAbs;
		if (type == "Polypropylene") return new CCleanPoly;
		if (type == "Polyethelene")  return new CCleanPoly;
		if (type == "PET" && metaltype == "Aluminum")        return new CCleanPetPoly;
		if (type == "PET" && metaltype == "Steel")           return new CCleanPetSteel;
		if (type == "Styrene")		return new CCleanSynthetics;
		if (type == "Nylon66")		return new CCleanSynthetics;
...
namespace strategy {		// DP 1.
	...
	public:
		...
		virtual int runTime(int orderSize, int cavities, string metal, int volume, string plastictype = "") = 0;
	...
	public:
		...
		int runTime(int orderSize, int cavities, string metal, int volume, string plastictype) 
		{ 
	...
	public:
		~Projection() { cout << "~Historical "; }
		int runTime(int orderSize, int cavities, string metal, int volume, string plastictype) {
			int cycletime = legacy_classes::cycleTime_sec(metal, volume);
			int rejectRate_pcnt = (plastictype == "PET") ? legacy_classes::rejectRateHigh_pcnt : legacy_classes::rejectRateLow_pcnt;
			return (float)(legacy_classes::setupAvg_min
	...
	RuntimeEstimate* RuntimeEstimate::makeObject(int order_size) {
		...
		if (order_size <= 100000) return new Projection();
...
namespace observer {		// DP 5.
	...
	class StufferObs : public BinObserver {
	public:
		StufferObs(string name, BinSubject* subject) : BinObserver(name, subject) {}
		~StufferObs() { DTORF("~StufferObs "); }
		void update() {
			if (m_name != "Air")
				cout << "        " << m_name << " stuffer triggered to fill " << m_subject->m_name << " parts bin." << endl; 
		}
	};
...
namespace abstract_factory {// DP 9.
	...
	class Machine_IJM140 : public Machine {
	public:
		~Machine_IJM140() { DTORF("~Machine_IJM140" ); }
		string metaltype() { return string("Aluminum"); } ;
		string metaltypeLowercase() { return string("aluminum"); } ;
		int numcavity() { return 4; };
		string belttype() { return string("V-Level conveyer belt"); };
		string binSubject() { return string("ShellBox"); };
		string machinename() { return string("IJM_140"); };
	};
	...
	class Machine_IJM220 : public Machine {
	public:
		~Machine_IJM220() { DTORF("~Machine_IJM220" ); }
		string metaltype() { return string("Steel"); } ;
		string metaltypeLowercase() { return string("steel"); } ;
		int numcavity() { return 2; };
		string belttype() { return string("Y-Split conveyer belt"); };
		string binSubject() { return string("PalletBox");} ;
		string machinename() { return string("IJM_220"); };
	};

	class Machine_IJM240 : public Machine {
	public:
		~Machine_IJM240() { DTORF("~Machine_IJM240" ); }
		string metaltype() { return string("Steel"); } ;
		string metaltypeLowercase() { return string("steel"); } ;
		int numcavity() { return 4; };
		string belttype() { return string("V-Level conveyer belt"); };
		string binSubject() { return string("Crate");} ;
		string machinename() { return string("IJM_240"); };
	};
	...
	class AbstractFactory_IJM140 : public InjectionLine {
	public:
		~AbstractFactory_IJM140() {}
		Machine* create_machine() { return new Machine_IJM140(); };
	};
	...
	class AbstractFactory_IJM220 : public InjectionLine {
	public:
		~AbstractFactory_IJM220() {}
		Machine* create_machine() { return new Machine_IJM220(); };
	};

	class AbstractFactory_IJM240 : public InjectionLine {
	public:
		~AbstractFactory_IJM240() {}
		Machine* create_machine() { return new Machine_IJM240(); };
	};
	...
	InjectionLine* InjectionLine::makeObject(int orderSize) {
		...
		if (orderSize <= 40000) return new AbstractFactory_IJM140;
		if (orderSize <= 50000) return new AbstractFactory_IJM210;
		if (orderSize <= 100000) return new AbstractFactory_IJM220;
		if (orderSize <= 200000) return new AbstractFactory_IJM240;
...
namespace bridge {			// DP 8.
	...
	PlatForm* PlatForm::makeObject(string type) {
		...
		if (type == "sanded") return new EDM;
		if (type == "satin") return new EDM;
	...
	class DinoAbs : public Shape {
	public:
		DinoAbs(PlatForm* plat) : Shape(plat) {};
		void mill(int cavity, string finish, string metal, string steps) { 
			m_plat->process("dino", cavity, finish, metal, steps); 
		}
	};
	...
	Shape* Shape::makeObject(string Shapetype, PlatForm* plat) {
		...
		if (Shapetype == "dino") return new DinoAbs(plat);
...
namespace chain_of_resp {	// DP 7.
	...
	public:
		void delegate(string &location, string mold, string &finish) {
			if (location == "sisterCompany")
				cout << "    Borrow " << mold << " mold from " << "sister company";
			else
				m_succesor->delegate(location, mold, finish);
		}
	};
	...
	public:
		void delegate(string &location, string mold, string &finish) {
			if (location == "purchase")
				cout << "    Acquire " << mold << " mold via purchase";
			else
				m_succesor->delegate(location, mold, finish);
		}
	};
	...
	GetMold* GetMold::makeObject(string t) {
		...
		if (t == "purchase") return new Purchase();
		if (t == "mill") return new Mill();
		return new Inventory();
	}

	GetMold* setUpChain() {
		const size_t chain_size = 4;
		string chain[chain_size] = { "inventory", "sisterCompany", "purchase", "mill" };
...
namespace decorator {		// DP 6.
	...
	public:
		...
		int m_volume;
		Recipe* m_decor;
		static Recipe* makeObject(Recipe* recp, string type, string name, int volume = 0, string color = "");
	};
	...
	public:
		PetDec(Recipe* decor, int volume, string color = "") : RecipeDecor(decor, volume) {}
		string desp() { return "PET(" + to_string(m_volume) + ") " + m_decor->desp(); }
		int recipe() { return m_decor->recipe() + m_volume; }
	};

	class StyreneDec : public RecipeDecor {
	public:
		StyreneDec(Recipe* decor, int volume, string color = "") : RecipeDecor(decor, volume), m_color(color) {}
		string desp() { return m_color + "-Styrene(" + to_string(m_volume) + ") " + m_decor->desp(); }
		int recipe() { return m_decor->recipe() + m_volume; }
		string m_color;
	};

	class Nylon66Dec : public RecipeDecor {
	public:
		Nylon66Dec(Recipe* decor, int volume, string color = "") : RecipeDecor(decor, volume), m_color(color) {}
		string desp() { return m_color + "-Nylon66(" + to_string(m_volume) + ") " + m_decor->desp(); }
		int recipe() { return m_decor->recipe() + m_volume; }
		string m_color;
	};
	...
	class MicroFibersDec : public RecipeDecor {
	public:
		MicroFibersDec(Recipe* decor, int volume) : RecipeDecor(decor, volume) {}
		string desp() { return "MicroFibers(" + std::to_string(m_volume) + ") " + m_decor->desp(); }
		int recipe() { return m_decor->recipe() + m_volume; }
	};

	Recipe* RecipeDecor::makeObject(Recipe* recp, string type, string name, int volume, string color) {
		...
		if (type == "plastic" && name == "Styrene") return new StyreneDec(recp, volume, color);
		if (type == "plastic" && name == "Nylon66") return new Nylon66Dec(recp, volume, color);
		...
		if (type == "MicroFibers") return new MicroFibersDec(recp, volume);
		return recp;
	}
...
namespace factory_method {	// DP 4.
	...
	public:
		...
		string name() { return "RecycleCode"; }
		int width() { return 6; }
	};
	...
	Tag* Tag::makeObject(string type) {
		...
		if (type == "IncCounter") return new IncCounter;
		if (type == "PartNumber") return new PartNumber;
		if (type == "RecycleCode") return new RecycleCode;
	...
	Mold* Mold::makeObject(string Shapetype) {
		...
		if (Shapetype == "dino") return new MoldDino();
		return new MoldDuck();
	}
	...
	class CZipLock : public Packager {
	public:
		~CZipLock() { DTORF("~CZipLock "); }
		void packing() { cout << " with ZipLock packager"; }
		string name() { return "ZipLock packager"; }
	};
	...
	Packager* Packager::makeObject(string packtype) {
		...
		if (packtype == "ZipLock") return new CZipLock;
		return new CPakBulk;
	}
// Seam point - add another class.
	...
	public:
		...
		virtual string wrap() { return ""; };
		static Stuffer* makeObject(string type);
	};

	class Air : public Stuffer {
	public:
		~Air() { DTORF("~Air "); }
		string name() { return "Air"; }
		string wrap() { return ""; }
	};
	...
	public:
		...
		string name() { return "Popcorn"; }
		string wrap() { return "styrene popcorn"; }
	};

	class Bubblewrap : public Stuffer {
	public:
		~Bubblewrap() { DTORF("~Bubblewrap "); }
		string name() { return "Bubblewrap"; }
		string wrap() { return "bubble wrap"; }
	};

	class Foam : public Stuffer {
	public:
		~Foam() { DTORF("~Foam "); }
		string name() { return "Foam"; }
		string wrap() { return "expanding foam"; }
	};

	Stuffer* Stuffer::makeObject(string type) {
		if (type == "Air") return new Air;
		if (type == "Popcorn") return new Popcorn;
		if (type == "Bubblewrap") return new Bubblewrap;
		if (type == "Foam") return new Foam;
		return new Air;
	}
}
...
namespace template_method {	// DP 3.
	...
	class InjectionSynthetics : public Injection {
		~InjectionSynthetics() { DTORF("~InjectionSynthetics "); }
	...
	Injection* Injection::makeObject(string plasticstype) {
		...
		if (plasticstype == "Styrene") return new InjectionSynthetics;
		if (plasticstype == "Nylon66") return new InjectionSynthetics;
...
int processTags(string tag, string &final_tag) {
	...
	while (!last_tag) {
		...
		if (token != "ModelNumber" && token != "Country" && token != "Date" && token != "IncCounter" && token != "PartNumber" && token != "RecycleCode")
...
void process(map<string,string>& order) {
	...
	if (order["stuffer"] == "") legacy_classes::defaulting(order, "stuffer", "None");
	int order_size = std::stoi(order["size"]);
	if (order_size > 200000) {
		cout << "  " << "<>Size exceeds mold lifetime |" << order_size << "| defaulting to HugeOrder of 200000.\n";
		order_size = 200000;
	}
	...
	factory_method::Stuffer * stuffer = factory_method::Stuffer::makeObject(order["stuffer"]);
	if (stuffer->name() != "Air")
		cout << " and " << stuffer->wrap() << " stuffer";
	cout << ":\n";
	...
	// Load plastic
	string plastic_name = order["plastic"];
	cout << "    " << "Load plastic bin with ";
	if ((order.find("color") != order.end()) && (plastic_name == "Styrene" || plastic_name == "Nylon66")) {
		plastic_name += " pellets";
		cout << order["color"] << "-";
	}
	cout << plastic_name;
	if (order.find("color") != order.end() && (order["plastic"] != "Styrene" && order["plastic"] != "Nylon66")) 
	...
	for (map<string, string>::iterator it = order.begin(); it != order.end(); it++) {
		...
		else if ((it->first == "color" && isColor(it->second)) && (order["plastic"] != "Styrene" && order["plastic"] != "Nylon66")) {
	...
	recipe = decorator::RecipeDecor::makeObject(recipe, "plastic", order["plastic"], plasctic_volume, order["color"]);
	...
	int timeestimate = runtime->runTime(order_size, machine->numcavity(), machine->metaltype(), mold->volume(), order["plastic"]);
	...
	observer::StufferObs stufObs(stuffer->name(), &binsubject);
	...
	// Adapter: cleanpaused
	adapter::CAdapterClean* myclean = adapter::CAdapterClean::makeObject(order["plastic"], machine->metaltype());
	myclean->clean();
