...
namespace final_original {
    ...
    namespace abstract_factory {// DP 9.
        ...
        public:
            ...
            ~SmallOrder() { DTORF(((size > 10000) ? "~SmallOrder " : "~PilotOrder ")); }
        ...
        public:
            ...
            void setupMold() {
                if (size < 20000)
                    mold = new Aluminum("IJM_110", "Aluminum", CAVITY_1);
                else
                    mold = new Aluminum("IJM_120", "Aluminum", CAVITY_2);
            }
        };
        ...
        public:
            ...
            void setupMold() {;
                mold = new Steel("IJM_210", "Steel", CAVITY_1);
            }
        };
        ...
        public:
            void setupIJM_Belt_Bin() {
                ijm_bin_belt = new IJM_Bin_Belt_Listerner("ShellBox");
                obsIJM = BinObserver::makeObject("IJM_140");
                obsBin = BinSubject::makeObject("ShellBox");
                obsBelt = BinObserver::makeObject("VLevelBelt");
                ijm_bin_belt->attach(obsIJM, obsBelt, obsBin);
            }
            void setupMold() { mold = new Aluminum("IJM_140", "Aluminum", CAVITY_4); }
        };
        ...
        public:
            void setupIJM_Belt_Bin() {
                if (size == 100000) {
                    ijm_bin_belt = new IJM_Bin_Belt_Listerner("PalletBox");
                    obsIJM = BinObserver::makeObject("IJM_220");
                    obsBin = BinSubject::makeObject("PalletBox");
                    obsBelt = BinObserver::makeObject("YSplitBelt");
                    ijm_bin_belt->attach(obsIJM, obsBelt, obsBin);
                }
                else {
                    ijm_bin_belt = new IJM_Bin_Belt_Listerner("Crate");
                    obsIJM = BinObserver::makeObject("IJM_240");
                    obsBin = BinSubject::makeObject("Crate");
                    obsBelt = BinObserver::makeObject("VLevelBelt");
                    ijm_bin_belt->attach(obsIJM, obsBelt, obsBin);
                }
            }
            void setupMold() {
                if (size == 100000)
                    mold = new Steel("IJM_220", "Steel", CAVITY_2);
                else
                    mold = new Steel("IJM_240", "Steel", CAVITY_2);
            }
        };
        ...
        public:
            ...
            virtual void ProcessMixing(map<string, string>& order, InjectionLine* inLine) {
                ...
                if (order["plastic"] == "Styrene" || order["plastic"] == "Nylon66")
                    cout << polymerHandle->getColor() << "-" << polymerHandle->getName() << " pellets";
                else {
                    cout << polymerHandle->getName();
                ...
                if (order["plastic"] == "Styrene" || order["plastic"] == "Nylon66") {
                    int shapeVol = inLine->getVolumeCC() - polymerHandle->getData();
                    cout << "      Recipe: " << polymerHandle->getColor() << "-" 
                        << polymerHandle->getName() << "(" << shapeVol << ") ";
                }
                else {
            ...
            virtual void ProcessRunTime(InjectionLine* inLine) {
                ...
                if (size <= SMALL_SIZE) rt = new BackOfTheEnvelope(size, cavity, metal, volume_cc, type);
                else if (size <= 25000) rt = new Calculation(size, cavity, metal, volume_cc, type);
                else if (size < 100000) rt = new Historical(size, cavity, metal, volume_cc, type);
                else {
                    if (size > 100000)
            ...
            virtual void ProcessShipping(map<string, string>& order) {
