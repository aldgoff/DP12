...
#define DTORF(x) if(on) { cout << x << " "; }
   ...
   namespace strategy {		// DP 1.
      ...
      RunTimeEstimator* RunTimeEstimator::GetInstance(int orderSize){
         ...
         else cout << "    <>Using most sophisticated estimation algorithm for orders greater than 100000.\n";
         return nullptr;
      }
   }
   ...
   namespace abstract_factory {// DP 9.
      ...
      InjectionLineFactory* InjectionLineFactory::GetInstance(int orderSize){
         ...
         if(20000 < orderSize && orderSize <= 50000) return new InjLineMediumOrderFactory;
         else assert(false);
         return nullptr;
      }
      ...
      class InjectionLine{
      public:
         InjectionLine(int orderSize, string packager){
   ...
   namespace chain_of_resp {	// DP 7.
      ...
      public:
         ...
         void GetMold(){
            cout << "    Borrow " << _mp.shape <<" mold from sister company.\n";
         }
         const string Name()const{ return "sister company"; }
      };
      ...
      MoldLocation* MoldLocation::GetInstance(MoldProperties& mp){
         ...
         else if(mp.moldLoc=="sister company") return new SisterCompany(mp);  
      ...
      const int NUM_MOLD_LOC = 3;
      MoldLocation* MoldLocation::SetupChain(const MoldProperties& mpIn){
         string chain[] = { "inventory", "sister company", "mill" }; // ToDo: scalability test, try deleting/adding 
   ...
   public:
      InjectionMoldingShop(Order& order){
         ...
         _injectionLine = new InjectionLine(orderSize, _order["packager"]);
