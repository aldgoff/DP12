...
int estimateRunTime(map<string,string>& order, int cavities, string metal, int volume) {
	...
	float rejectRate_pcnt = order["plastic"] == "PET" ? legacy_classes::rejectRateHigh_pcnt : legacy_classes::rejectRateLow_pcnt;
	float runTime = (legacy_classes::setupAvg_min
...
class Sythetics : public CleanMold {
	legacy_classes::PETCleanup wrapped;
public:
	Sythetics(string& name) : CleanMold(name) {}
	~Sythetics() { DTORF("~Sythetics "); }
	void Clean(string& metal) {
...
CleanMold* CleanMold::createObject(map<string,string>& order) {
	...
	if ((plasticType == "Styrene") || (plasticType=="Nylon66")) return new Sythetics(plasticType);
...
public:
	RuntimeEstimate(map<string,string>& order) : order(order), orderSize(stoi(order["size"])) {}
	virtual ~RuntimeEstimate() { DTORF("~strategy::RuntimeEstimate\n"); }
	virtual float Calc(int cavities, string metal, int volume) = 0;
};
class BackOfTheEnvelope : public RuntimeEstimate {
public:
	BackOfTheEnvelope(map<string,string>& order) : RuntimeEstimate(order) {}
	virtual ~BackOfTheEnvelope() { DTORF("~BackOfTheEnvelope "); }
	float Calc(int cavities, string metal, int volume) {
		return (float)orderSize/cavities*((float)1/60);
	}
};
class Calculation : public RuntimeEstimate {
public:
	Calculation(map<string,string>& order) : RuntimeEstimate(order) {}
	virtual ~Calculation() { DTORF("~Calculation "); }
	float Calc(int cavities, string metal, int volume) {
		return ((float)orderSize/cavities)*(float)legacy_classes::cycleTime_sec(metal, volume)/3600;
	}
};
class Historical : public RuntimeEstimate {
public:
	Historical(map<string,string>& order) : RuntimeEstimate(order) {}
...
public:
	Projection(map<string,string>& order) : RuntimeEstimate(order) {}
...
class Crate : public BinSubject {
public:
	Crate() : BinSubject("Crate") {}
	~Crate() { DTORF("~Crate "); }
};
...
public:
	...
	virtual void pause() {
		cout << "        " << this->getName() << " paused while " << bin.getName()<< " parts bin was swapped.\n";
	}
};
...
public:
	IJM_140(observer::BinSubject& bin) : IJM(bin) {}
	virtual ~IJM_140() { DTORF("~IJM_140 "); }
	string getName() { return "IJM_140"; }
};
...
public:
	IJM_220(observer::BinSubject& bin) : IJM(bin) {}
	virtual ~IJM_220() { DTORF("~IJM_220 "); }
	string getName() { return "IJM_220"; }
};
class IJM_240 : public IJM {
public:
	IJM_240(observer::BinSubject& bin) : IJM(bin) {}
	virtual ~IJM_240() { DTORF("~IJM_240 "); }
	string getName() { return "IJM_240"; }
};
...
public:
	VLevelBelt(observer::BinSubject& bin) : ConveyerBelt(bin) {}
	~VLevelBelt() { DTORF("~VLevelBelt "); }
	string getName() { return "V-Level conveyer belt"; }
};
...
public:
	...
	observer::BinSubject* createPartsBin() { return bin = new  observer::ShellBox; }
	IJM* createIJM(){ return new IJM_140(*bin); }
	Mold* createMold() { return new Aluminum(4); }
	ConveyerBelt* createConveyerBelt() { return new VLevelBelt(*bin); }
};
...
public:
	...
	observer::BinSubject* createPartsBin() { return bin = new  observer::PalletBox; }
	IJM* createIJM(){ return new IJM_220(*bin); }
	Mold* createMold() { return new Steel(2); }
	ConveyerBelt* createConveyerBelt() { return new YSplitBelt(*bin); }
};
...
public:
	...
	observer::BinSubject* createPartsBin() { return bin = new  observer::Crate; }
	IJM* createIJM(){ return new IJM_240(*bin); }
	Mold* createMold() { return new Steel(4); }
	ConveyerBelt* createConveyerBelt() { return new VLevelBelt(*bin); }
};
...
InjectionLine* InjectionLine::makeObject(string& orderSize) {
	...
	else if (ordSize <=40000) return new FastOrder;
	else if (ordSize <= 50000) return new MediumOrder;
	else if (ordSize <= 100000) return new LargeOrder;
	else if (ordSize > 200000) {
...
public:
	~Dino() { DTORF("~Dino "); };
	string getName() {return "dino";}
	int getVol() {return 30;}
	string getMillSteps() {return "G5D2C1";}
};
...
Shape* Shape::createObject(map<string,string>& order) {
	...
	if (shapeName == "dino") return new Dino();
	if (shapeName != "duck") {
		cout << "  ";
...
public:
	EDM(abstract_factory::Mold* moldType, bridge::Shape* shape, string finish) :
		Platform(moldType, shape, "EDM tools (static punch, static slice, and static etch)", finish) {} 
	~EDM() { DTORF("~EDM "); }
};
...
Platform* Platform::makeObject(map<string,string>& order, bridge::Shape* shape, abstract_factory::Mold* moldType) {
	...
	if (finish == "sanded") return new EDM(moldType, shape, finish);
	if (finish == "satin") return new EDM(moldType, shape, finish);
	throw "error";
}
...
public:
	...
	void Get(map<string,string>& order, bridge::Shape* shape, abstract_factory::Mold* moldType) {
		if (order["moldLoc"] == "sisterCompany") cout << "    Borrow " << shape->getName() << " mold from sister company.\n";
		else sucessor->Get(order, shape, moldType);
	}
};
...
public:
	...
	void Get(map<string,string>& order, bridge::Shape* shape, abstract_factory::Mold* moldType) {
		if (order["moldLoc"] == "purchase") cout << "    Acquire " << shape->getName() << " mold via purchase.\n";
		else sucessor->Get(order, shape, moldType);
	}
};
...
GetMold* setupLocationChain() {
	...
	GetMold* purchase = new Purchase();
	GetMold* mill = new Mill();
	inventory->setSucessor(sisterCompany);
	sisterCompany->setSucessor(purchase);
	purchase->setSucessor(mill);
...
public:
	IncCounter(Cavity* decorator) : Tags(decorator) {}
	~IncCounter() { DTORF("~IncCounter "); }
	string getName() {
		string all = decorator->getName();
		if (all != "") all = all + " ";
		return all + "IncCounter";
	}
	int getWidth() { return 4 + decorator->getWidth(); }
};
class PartNumber : public Tags {
public:
	PartNumber(Cavity* decorator) : Tags(decorator) {}
	~PartNumber() { DTORF("~PartNumber "); }
	string getName() {
		string all = decorator->getName();
		if (all != "") all = all + " ";
		return all + "PartNumber";
	}
	int getWidth() { return 2 + decorator->getWidth(); }
};
class RecycleCode : public Tags {
public:
	RecycleCode(Cavity* decorator) : Tags(decorator) {}
	~RecycleCode() { DTORF("~RecycleCode "); }
	string getName() {
		string all = decorator->getName();
		if (all != "") all = all + " ";
		return all + "RecycleCode";
	}
	int getWidth() { return 6 + decorator->getWidth(); }
};
...
Cavity* Tags::makeObject(Cavity* decorator, string& criteria) {
	...
	if (criteria == "IncCounter") return new IncCounter(decorator);
	if (criteria == "PartNumber") return new PartNumber(decorator);
	if (criteria == "RecycleCode") return new RecycleCode(decorator);
...
Polymer* Polymer::Create(map<string,string>& order, int volume) {
	...
	if( order["MicroFibers"] != "") recipe = new Ingredient(recipe, "MicroFibers", stoi(order["MicroFibers"]));
	...
	if (order["plastic"] == "Styrene" || order["plastic"] == "Nylon66") recipe = new Ingredient(recipe, order["color"] + "-" + order["plastic"], volume - recipe->getVolume());
	else {
...
public:
	ZipLock(observer::BinSubject& bin) : Packager(bin) {}
	~ZipLock() { DTORF("~ZipLock "); }
	string getName() { return "Ziplock packager"; }
	string getNameCamelCase() { return "ZipLock packager"; }
};
// Seam point - add another algorithm.
...
Packager* Packager::createObject(map<string,string>& order, observer::BinSubject& bin) {
	...
	else if (packagerType=="ZipLock") return new factory_method::ZipLock(bin);
...
class Stuffer : public observer::BinObserver {
public:
	Stuffer(observer::BinSubject& bin) : BinObserver(bin) {}
	...
	void pause() {
		cout << "        " << this->getName() << " triggered to fill " << bin.getName()<< " parts bin.\n";
	}
	static Stuffer* createObject(map<string,string>& order, observer::BinSubject& bin);
};

class Popcorn : public Stuffer {
public:
	Popcorn(observer::BinSubject& bin) : Stuffer(bin) {}
	~Popcorn() { DTORF("~Popcorn "); }
	string getName() { return "Popcorn stuffer"; }
	string getNameCamelCase() { return "styrene popcorn stuffer"; }
};

class Bubblewrap : public Stuffer {
public:
	Bubblewrap(observer::BinSubject& bin) : Stuffer(bin) {}
	~Bubblewrap() { DTORF("~Bubblewrap "); }
	string getName() { return "Bubblewrap stuffer"; }
	string getNameCamelCase() { return "bubble wrap stuffer"; }
};

class Foam : public Stuffer {
public:
	Foam(observer::BinSubject& bin) : Stuffer(bin) {}
	~Foam() { DTORF("~Foam "); }
	string getName() { return "Foam stuffer"; }
	string getNameCamelCase() { return "expanding foam stuffer"; }
};

Stuffer* Stuffer::createObject(map<string,string>& order, observer::BinSubject& bin) {
	string type = order["stuffer"];
	if (type=="Popcorn") return new factory_method::Popcorn(bin);
	else if (type=="Bubblewrap") return new factory_method::Bubblewrap(bin);
	else if (type=="Foam") return new factory_method::Foam(bin);
	else if (type != "Air") {
...
private:
	...
	factory_method::Stuffer *stuffer;
...
public:
	...
	void Setup (map<string,string>& order) {
		...
		stuffer = factory_method::Stuffer::createObject(order, *partsBin);
		
		cout << "  Setup injection line for " << order["size"] <<" order";
		if (packager) cout << " with "<< packager->getNameCamelCase();
		if (stuffer) cout << " and "<< stuffer->getNameCamelCase();
		cout << ":\n";
	...
	void LoadPlasticBin (map<string,string>& order) {
		...
		if (order["plastic"] == "Styrene" || order["plastic"] == "Nylon66") cout << order["color"] << "-" << order["plastic"] << " pellets";
		else {
			cout << order["plastic"];
	...
	void EstimateTime(map<string,string>& order) {
		...
		int runTime = estimateRunTime(order, moldType->getCavities(), moldType->getMetalLowercase(), shape->getVol());
